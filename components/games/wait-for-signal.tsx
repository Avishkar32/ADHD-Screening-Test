...
//// Define Signal type or variable before checking
...
//var GameMetrics;
//
//if (typeof GameMetrics === 'undefined') {
//  GameMetrics = {
//    age: 0,
//    adhd_status: 0,
//    playtime_min: 0,
...
//  const [rocket, setRocket] = useState({ launched: false, y: 0, speed: 0 })
...
//        signalSize: 100,
//        gameDuration: 120000, // 2 minutes max
//        rocketSpeed: 5,
//        targetLaunches: 3,
//      }
...
//            
//            // Increment successful launches
//            setSuccessfulLaunches(prev => prev + 1)
//          } else {
//            // False positive (pressed on red)
//            setFalsePositives((prev) => prev + 1)
//          }
//        } else {
//          // Premature response (pressed when no signal)
...
//
//  // Handle canvas click (for touch devices)
...
//      setGameTime(elapsed)
//
//      // Check if game time is up or target launches reached
...
//    }
//
//    // Create nebula colors for background
//    const createNebula = (x: number, y: number, radius: number, colors: any[]) => {
...
//
//      // Draw space background - gradient with deep colors
...
//
//      // Draw nebulas
//      const nebula1 = createNebula(width * 0.7, height * 0.2, width * 0.4, 
//        ["rgba(75, 0, 130, 0)", "rgba(75, 0, 130, 0.1)", "rgba(75, 0, 130, 0)"])
//      ctx.fillStyle = nebula1
//      ctx.fillRect(0, 0, width, height)
//
//      const nebula2 = createNebula(width * 0.3, height * 0.7, width * 0.5, 
//        ["rgba(0, 0, 255, 0)", "rgba(0, 0, 255, 0.05)", "rgba(0, 0, 255, 0)"])
//      ctx.fillStyle = nebula2
//      ctx.fillRect(0, 0, width, height)
//
//      // Animate and draw stars with twinkling effect
...
//        
//        // Twinkle effect
//        star.opacity = 0.2 + Math.abs(Math.sin(Date.now() * 0.001 + i)) * 0.8
//        
...
//      )
//      planetGradient.addColorStop(0, "#a16bca")
//      planetGradient.addColorStop(0.7, "#7a3aad")
//      planetGradient.addColorStop(1, "#5e2d89")
//      
...
//      ctx.rotate(planet.rings.angle)
//      ctx.scale(1, 0.2)
//      
//      const ringGradient = ctx.createRadialGradient(
//        0, 0, planet.rings.innerRadius,
//        0, 0, planet.rings.outerRadius
//      )
//      ringGradient.addColorStop(0, "rgba(230, 200, 255, 0.1)")
//      ringGradient.addColorStop(0.5, "rgba(230, 200, 255, 0.3)")
//      ringGradient.addColorStop(1, "rgba(230, 200, 255, 0.05)")
//      
//      ctx.fillStyle = ringGradient
//      ctx.beginPath()
//      ctx.arc(0, 0, planet.rings.outerRadius, 0, Math.PI * 2)
//      ctx.arc(0, 0, planet.rings.innerRadius, 0, Math.PI * 2, true)
//      ctx.fill()
//      
//      ctx.restore()
//      ctx.restore()
//
//      // Draw launch platform - futuristic design
//      const platformWidth = 200
//      const platformHeight = 60
//      
//      // Platform base with glowing effect
//      const platformGradient = ctx.createLinearGradient(
//        width / 2 - platformWidth / 2, height - platformHeight,
//        width / 2 + platformWidth / 2, height - platformHeight
//      )
...
//      ctx.stroke()
//      
//      // Launch pad circle with pulsing effect
//      const pulseSize = 2 + Math.sin(Date.now() * 0.005) * 2
//      const launchCircleGradient = ctx.createRadialGradient(
//        width / 2, height - platformHeight - 2,
//        0,
//        width / 2, height - platformHeight - 2,
//        30 + pulseSize
//      )
...
//      ctx.fill()
//
//      // Draw signal - more advanced with glow effects
//      if (signal.active) {
...
//          signalX, signalY, 0,
//          signalX, signalY, config.signalSize * 1.5
//        )
//        
//        if (signal.type === "green") {
...
//        ctx.fill()
//        
//        // Main signal circle with gradient
//        const signalGradient = ctx.createRadialGradient(
//          signalX, signalY, 0,
//          signalX, signalY, config.signalSize
//        )
//        
//        if (signal.type === "green") {
...
//        
//        // Pulse ring animation around signal
...
//        const rocketY = height - 60 - rocket.y
//        ctx.save()
...
//        } else {
//          // Reset rocket after launch
//          setTimeout(() => {
//            setRocket({ launched: false, y: 0, speed: config.rocketSpeed })
//          }, 500)
//          return prev
//        }
//      })
...
//    const metrics = {
//      age,
//      adhd_status: 0, // Placeholder
//      playtime_min: Math.round(gameTime / 60000),
//      session_incomplete: successfulLaunches < targetLaunches ? 1 : 0,
//      wfs_fpr: falsePositives,
//      wfs_prc: prematureResponses,
//      wfs_rt: reactionTimes.length > 0 ? Math.round(reactionTimes.reduce((a, b) => a + b, 0) / reactionTimes.length) : 0,
//      wfs_gs: gazeShifts,
...
export default function WaitForSignal({ age, onComplete }: WaitForSignalProps) {
  ...
  useEffect(() => {
    ...
    const planet = { x: width * 0.15, y: height * 0.3, radius: width * 0.08, rotation: 0, rings: { innerRadius: width * 0.1, outerRadius: width * 0.15, angle: Math.PI / 6 } }
    ...
    const render = () => {
      ...
      const nebula1 = createNebula(width * 0.7, height * 0.2, width * 0.4, ["rgba(75, 0, 130, 0)", "rgba(75, 0, 130, 0.1)", "rgba(75, 0, 130, 0)"])
      ctx.fillStyle = nebula1
      ctx.fillRect(0, 0, width, height)

      const nebula2 = createNebula(width * 0.3, height * 0.7, width * 0.5, ["rgba(0, 0, 255, 0)", "rgba(0, 0, 255, 0.05)", "rgba(0, 0, 255, 0)"])
      ctx.fillStyle = nebula2
      ctx.fillRect(0, 0, width, height)
      ...
      stars.forEach((star) => {
        ...
        star.opacity = 0.2 + Math.abs(Math.sin(Date.now() * 0.001 + stars.indexOf(star))) * 0.8
        ctx.fillStyle = `rgba(255, 255, 255, ${star.opacity})`
        ctx.beginPath()
        ctx.arc(star.x, star.y, star.size, 0, Math.PI * 2)
        ctx.fill()
      ...
      ctx.rotate(planet.rings.angle)
      ctx.scale(1, 0.2)
      const ringGradient = ctx.createRadialGradient(0, 0, planet.rings.innerRadius, 0, 0, planet.rings.outerRadius)
      ringGradient.addColorStop(0, "rgba(230, 200, 255, 0.1)")
      ringGradient.addColorStop(0.5, "rgba(230, 200, 255, 0.3)")
      ringGradient.addColorStop(1, "rgba(230, 200, 255, 0.05)")
      ctx.fillStyle = ringGradient
  ...
  useEffect(() => {
    ...
    console.log("Calculated Metrics:", normalizedMetrics)
    if (onComplete) onComplete(normalizedMetrics)
  }, [gameState])
